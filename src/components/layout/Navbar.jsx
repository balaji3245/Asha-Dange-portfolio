      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-20">
          ...
          <div className="flex-shrink-0 flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo(0,0)}>
            <div className="p-1.5 sm:p-2 bg-secondary/10 dark:bg-accent/10 rounded-lg text-secondary dark:text-accent flex-shrink-0">
              <BookOpen size={20} className="sm:hidden" />
              <BookOpen size={24} className="hidden sm:block" />
            </div>
            <span className="font-bold text-base sm:text-xl tracking-tight text-slate-800 dark:text-white">
              Asha Dange
            </span>
          </div>
            ...
            <div className="flex space-x-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="nav-link text-sm font-medium text-slate-600 hover:text-secondary dark:text-slate-300 dark:hover:text-accent transition-colors"
                >
                  {link.name}
                </a>
              ))}
